...
#ifndef __minix
/* MEECHO: RPC в системе нет, и этому файлу он не нужен - ни одного
 * обращения к rpc/xdr в нём нет, только сам include. */
#include <rpc/rpc.h>
#endif
...
#ifdef __minix
				/*
				 * MEECHO: поля ifa_addrflags в struct ifaddrs
				 * нет - getifaddrs(3) здесь его не заполняет.
				 * Значит отсеять tentative, detached и
				 * deprecated адреса IPv6, а равно и выбрать
				 * временный, нечем. Задевает только выбор
				 * исходного адреса по BindInterface для IPv6.
				 */
				(void)prefertemp;
#else
