...
# мёртвой до человека.
#
# Три ступени, а не две. Пятнадцать минут - под прогон по расписанию, но мало
# под работу руками по ssh, а снятый сторож (no_bootwd) означает, что
# зависшую плату вернёт только человек у выключателя. Поэтому между ними есть
# час: сессии хватает, спасение остаётся. no_bootwd проверяется первым - он
# сильнее.
if test -e mmc ${devnum}:${distro_bootpart} meecho/no_bootwd; then
	echo "MEECHO: boot watchdog off (meecho/no_bootwd present)"
elif test -e mmc ${devnum}:${distro_bootpart} meecho/bootwd_long; then
