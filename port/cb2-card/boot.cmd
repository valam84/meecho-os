...
# Здесь стояло объяснение «ядро встаёт посреди баннера с того прогона, где
# поднялись вторичные ядра» — оно оказалось неверным: тот симптом был лавиной
# прерываний UART (PORTING-LOG.md, «Плата: 8.3 сдана»), и no_smp его не
# менял. Работает ли SMP на плате, не проверено; файл оставлен как мера
# предосторожности до отдельного прогона, а не как диагноз.
