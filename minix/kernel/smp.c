...
struct sched_ipi_data {
	...
	/*
	 * A struct proc *, so it is as wide as a pointer. It was u32_t, which
	 * is the same thing only on a 32-bit machine.
	 */
	volatile vir_bytes data;
};
...
static void smp_schedule_sync(struct proc * p, unsigned task)
{
	...
	sched_ipi_data[cpu].data = (vir_bytes) p;
